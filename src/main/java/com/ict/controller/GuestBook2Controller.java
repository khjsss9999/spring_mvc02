...
public class GuestBook2Controller {
	...
	// spring-security.xml을 만들어서 bean 생성
	// web.xml에서 지정해 줘야 spring-security.xml을 읽을 수 있도록 지정
	@Autowired
	...
	public ModelAndView getGuestBook2WriteOK(VO vo, HttpServletRequest request) {
		try {
			...
			if(file.isEmpty()) {
				...
				// 그러나 같은 이름의 파일이 있으면 파일을 업로드가 안되므로 
	...
	public ModelAndView getGuestBook2DeleteOK(VO vo) {
		...
		// passwordEncoder.matches(암호화되지 않은 것, 암호화가 된것)
		// 일치하면 true, 아니면 false
